...
class GaborLayer(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, 
        ...
        bias2=False, relu=True, use_alphas=True, sigma_thresh=1e8):
        ...
        self.sigmas = nn.Parameter(torch.randn(self.total_kernels).unsqueeze(
            dim=1).unsqueeze(dim=2).unsqueeze(dim=3))
        self.below_sigmas = torch.zeros_like(self.sigmas, dtype=torch.bool)
    ...
    def generate_gabor_kernels(self):
        ...
        # See which sigmas have gone beyond the threshold
        curr_below = self.sigmas < self.sigma_thresh
        # update which sigmas
        self.below_sigmas = self.below_sigmas.to(self.sigmas.device)
        self.below_sigmas = self.below_sigmas | curr_below
        # update sigmas themselves
        self.sigmas = nn.Parameter(self.sigmas * self.below_sigmas)
        # update the kernels generated by those sigmas
        ori_gb = ori_gb * self.below_sigmas
